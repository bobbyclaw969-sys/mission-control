...
const agents: Agent[] = [
  ...
  { id: '5', name: 'Sam', role: 'Designer', status: 'working', specialty: 'Web Design', code: 'web-designer' },
];

const tasks: Task[] = [
  { id: '1', title: 'Query SF Permit API for new leads', status: 'in-progress', assignee: 'Sarah', priority: 'high' },
  { id: '2', title: 'Email contractors from leads', status: 'todo', assignee: 'Sarah', priority: 'high' },
  { id: '3', title: '$10K Challenge - Multiple income streams', status: 'in-progress', assignee: 'Team', priority: 'high' },
  { id: '4', title: 'Write OpenClaw Handbook for KDP', status: 'done', assignee: 'Jordan', priority: 'medium' },
...
const projects = [
  { name: 'SF Permit Finder', desc: 'Real-time building permits from SF DBI', url: 'https://bobbyclaw969-sys.github.io/sf-permit-finder/', category: 'Website', status: 'Live', visits: '1.2K' },
  { name: 'Santa Cruz Permit Finder', desc: 'GIS-based permit data', url: 'https://bobbyclaw969-sys.github.io/scc-permit-finder/', category: 'Website', status: 'Live', visits: '450' },
  { name: 'BuilderQuote Pro', desc: 'Contractor quote generation tool', url: 'https://bobbyclaw969-sys.github.io/argoquote/', category: 'Tool', status: 'Live', visits: '890' },
  { name: 'Mission Control', desc: 'AI agent operations dashboard', url: 'https://bobbyclaw969-sys.github.io/mission-control/', category: 'Tool', status: 'Live', visits: '2.1K' },
  { name: 'ARGO Logistics Demo', desc: 'AI agent demo for logistics', url: 'https://bobbyclaw969-sys.github.io/argo-ai-demo/', category: 'Demo', status: 'Demo', visits: '320' },
  { name: 'Edge Aura Pro', desc: 'Android app for IoT control', url: 'https://github.com/bobbyclaw969-sys/edge-aura-pro', category: 'App', status: 'Live', visits: 'N/A' },
];

export default function MissionControl() {
  const [activeTab, setActiveTab] = useState<'team' | 'projects' | 'tasks' | 'memory'>('tasks');

  const getAgentTasks = (agentName: string) => tasks.filter(t => t.assignee === agentName);
  const getTeamTasks = () => tasks.filter(t => t.assignee === 'Team');
  ...
  return (
    <div className="min-h-screen bg-[#030304] text-white font-sans">
      ...
      <div className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="max-w-6xl mx-auto px-8 pt-12 pb-8 relative">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="text-white/40 text-sm">Revenue Today</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4 mt-8">
            {[
              { label: 'Active Agents', value: '5', icon: '🤖', color: 'from-violet-600 to-purple-600' },
              { label: 'Tasks Running', value: '4', icon: '⚡', color: 'from-emerald-600 to-teal-600' },
              { label: 'Projects', value: '6', icon: '🚀', color: 'from-blue-600 to-cyan-600' },
              { label: '$10K Challenge', value: 'Day 1', icon: '🎯', color: 'from-amber-600 to-orange-600' },
            ].map((stat, i) => (
      ...
      <div className="max-w-6xl mx-auto px-8 py-10">
        ...
        {activeTab === 'tasks' && (
          <div className="space-y-6">
            ...
            {/* High Priority */}
            <div>
              <h3 className="text-sm font-medium text-red-400 mb-3 uppercase tracking-wider">High Priority</h3>
              <div className="space-y-2">
                {tasks.filter(t => t.priority === 'high').map(task => (
                  <div key={task.id} className="bg-white/[0.03] hover:bg-white/[0.05] rounded-xl p-4 border border-white/5 flex items-center gap-4">
                    <div className={`w-3 h-3 rounded-full ${
                      task.status === 'done' ? 'bg-emerald-500' : 
                      task.status === 'in-progress' ? 'bg-amber-500' : 'bg-gray-500'
                    }`} />
                    <span className="flex-1">{task.title}</span>
                    <span className="text-white/40 text-sm">{task.assignee}</span>
                    <span className={`px-2.5 py-1 rounded-lg text-xs ${getPriorityColor(task.priority)}`}>
                      {task.priority}
                    </span>
                  </div>
                ))}
            ...
            {/* Other Tasks */}
            <div>
              <h3 className="text-sm font-medium text-white/40 mb-3 uppercase tracking-wider">Other Tasks</h3>
              <div className="space-y-2">
                {tasks.filter(t => !t.priority || t.priority !== 'high').map(task => (
                  <div key={task.id} className="bg-white/[0.02] hover:bg-white/[0.03] rounded-xl p-4 border border-white/5 flex items-center gap-4 opacity-60">
                    <div className={`w-3 h-3 rounded-full ${
                      task.status === 'done' ? 'bg-emerald-500' : 
                      task.status === 'in-progress' ? 'bg-amber-500' : 'bg-gray-500'
                    }`} />
              ...
              return (
                <div key={agent.id} className="group bg-gradient-to-r from-white/[0.04] to-transparent rounded-2xl p-6 border border-white/5 hover:border-white/10 transition-all">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="w-80">
                      <p className="text-white/30 text-xs uppercase tracking-wider mb-2">Tasks ({agentTasks.length})</p>
                      <div className="flex flex-wrap gap-2">
                        {agentTasks.map(task => (
                          <span key={task.id} className={`text-xs px-2.5 py-1 rounded-lg ${
                            ...
                            {task.status === 'done' ? '✓' : task.status === 'in-progress' ? '◐' : '○'} {task.title.slice(0, 25)}...
                          </span>
                        ))}
                        {agentTasks.length === 0 && <span className="text-white/20 text-xs">No active tasks</span>}
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((project, i) => (
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <span className="text-white/30 text-xs">👁 {project.visits}</span>
        ...
        {activeTab === 'memory' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { title: '$10K Challenge Started', date: 'Today', preview: 'Multiple income streams launched', tag: 'Strategy' },
              { title: 'Conway VM Running', date: 'Feb 21', preview: '1 active VM on Conway', tag: 'Infrastructure' },
              { title: 'Book Written', date: 'Feb 22', preview: 'OpenClaw Handbook ready for KDP', tag: 'Content' },
              { title: 'Team Named', date: 'Feb 22', preview: 'Sarah, Mike, Alex, Jordan, Sam', tag: 'Team' },
            ].map((mem, i) => (
